// ============================================
//  Epicor PR Automation - Frontend Logic
// ============================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    excelFileInput.addEventListener('change', () => {
        if (excelFileInput.files.length > 0) {
            selectFile(excelFileInput.files[0]);
        }
    });
    ...
    function connectSSE() {
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                } else if (data.type === 'pr_number') {
                    showPRNumber(data.message);
                }
            } catch (e) {
        ...
        eventSource.onerror = () => {
            appendLog('[WARN] Lost connection to server. Reconnecting...', 'warn');
        };
    }
    ...
    runBtn.addEventListener('click', async () => {
        ...
        if (!username || !password) {
            appendLog('[ERROR] Please enter both Username and Password.', 'error');
            return;
        }
        if (!selectedFile) {
            appendLog('[ERROR] Please upload an Excel file.', 'error');
            return;
        }
        if (!noValues) {
            appendLog('[ERROR] Please enter at least one row "No" value.', 'error');
            return;
        }
